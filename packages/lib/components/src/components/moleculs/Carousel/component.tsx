...
export const Carousel = compose<CarouselProps, CarouselProps>(
  ...
  tap((props: CarouselProps) => {
    ...
    log.info(
      `Props are ${JSON.stringify(props, null, 2)}`,
      Carousel.displayName as string
    );

    if (typeof variant === 'undefined') {
      log.error(
        'Prop variant has to be defined',
        Carousel.displayName as string
      );
    }
    ...
    if (variant === Variants.MULTI) {
      !Array.isArray(slides) &&
        log.error(
          'Since Variant.MULTI you have to provide multi array.',
          Carousel.displayName as string
        );
    } else if (variant === Variants.SIMPLE) {
      !Array.isArray(slides) &&
        log.error(
          'Since Variant.SIMPLE you have to provide simple array.',
          Carousel.displayName as string
        );
    }
  })
